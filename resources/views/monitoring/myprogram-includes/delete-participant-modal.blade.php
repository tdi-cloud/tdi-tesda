<dialog id="delete_part_modal" class="modal">
  <div class="modal-box rounded-2xl p-8 transform transition-all animate-[fadeIn_0.2s_ease-out] max-w-md w-full">
     ...
     <p id="modalMessage" class="text-slate-500 dark:text-slate-200 text-center mb-8">Do you want to delete this participant?</p><!-- Buttons -->

     <input type="hidden" id="delete_part_id">
     <div class="flex gap-3">
        
        <button onclick="delete_part_modal.close()" class="flex-1 py-3 px-4 rounded-xl border-2 border-slate-200 text-slate-600 font-semibold hover:bg-slate-50 transition-all"> No </button> 

        <button onclick="deleteParticipant()" id="yes_del_part_btn" class="flex-1 py-3 px-4 rounded-xl bg-red-500 text-white font-semibold hover:bg-red-600 transition-all"> Yes </button>
     </div>
