<x-layout>
    ...
    <section class="w-full">
        ...
        <div class="px-10 py-4 border-b border-slate-300 dark:border-slate-600">
            <h1 class="poppins-bold text-2xl bg-gradient-to-r from-indigo-800 to-indigo-700 bg-clip-text text-transparent" >Erolled Programs</h1>
        </div>

        {{-- FILTER AREA --}}
        <div class="px-10 py-4 flex gap-2">
...
<script>
    ...
    function loadPrograms(page = 1, query = '', perPage = 10) {
        fetch(`/my-programs?q=${query}&page=${page}&per_page=${perPage}`)
            .then(res => res.json())
            .then(data => {
    ...
    function changePage(page) {
            currentPage = page;
            loadPrograms(currentPage, currentQuery, perPage);
        }
